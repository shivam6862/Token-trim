...
const useUpdateparentchatById = () => {
  const chatBotCtx = useContext(ChatBotContext);
  const { NotificationHandler } = useNotification();
  const UpdateparentchatById = async (
    chatId: string,
    message: string,
    files: any
  ) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/tokentrim/chat/${chatId}/`,
        {
          method: "POST",
          body: formData,
        }
  ...
  return { UpdateparentchatById };
};

export default useUpdateparentchatById;
