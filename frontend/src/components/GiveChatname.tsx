...
import classes from "@/styles/GiveChatname.module.css";
...
const GiveChatname: React.FC<GiveChatnameProps> = ({
  ...
  const { CreateNewparentchat } = useCreateNewparentchat();
  ...
  const makeNewChat = async () => {
    ...
    const response = (await CreateNewparentchat(name)) as any;
